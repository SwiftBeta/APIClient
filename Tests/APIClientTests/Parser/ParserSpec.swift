...
class ParserSpec: XCTestCase {
    ...
    func testParser_WhenCorrectJSONisMappedIntoAModel_ShouldHaveCorrectValuesInProperties() throws {
        ...
        // Act
        let sut = try Parser().parse(data, type: SwiftBetaModel.self, decoder: .init())
    ...
    func testParser_WhenKeyNotFoundInJSON_ShouldThrowsAndException() throws {
        ...
        XCTAssertThrowsError(try Parser().parse(data, type: SwiftBetaModel.self, decoder: .init()), "JSON Key Not Found") { error in
    ...
    func testParser_WhenMismatchTypeInJSON_ShouldThrowsAndException() throws {
        ...
        XCTAssertThrowsError(try Parser().parse(data, type: SwiftBetaModel.self, decoder: .init()), "JSON Type Mismatch") { error in
            XCTAssertTrue(error is DecodingError)
        }
    }
}

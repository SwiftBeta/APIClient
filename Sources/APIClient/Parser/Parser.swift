...
public protocol ParserProtocol {
    func parse<T: Decodable>(_ data: Result<Data, Error>, type: T.Type) throws -> T?
    func parse<T: Decodable>(_ data: Data, type: T.Type, decoder: JSONDecoder) throws -> T?
}

final public class Parser: ParserProtocol {
    public init() { }
    
    public func parse<T: Decodable>(_ result: Result<Data, Error>, type: T.Type) throws -> T? {
        switch result {
        case .success(let data):
            let dataModel = try parse(data, type: T.self)
            return dataModel
    ...
    public func parse<T: Decodable>(_ data: Data, type: T.Type, decoder: JSONDecoder = .init()) throws -> T? {
        do {
